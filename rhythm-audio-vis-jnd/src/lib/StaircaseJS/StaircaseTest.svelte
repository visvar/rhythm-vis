            ...
            while (position === false && loop < 100) {
                position = {
                    x:
                        ...
                        Math.round(
                            Math.random() *
                                (ctx.canvas.clientWidth - 2 * radius)
                        ),
                    y:
                        radius +
                        Math.round(
                ...
                if (
                    ...
                    for (var n = 0; n < circles.length; n++) {
                        if (
                            Math.abs(position.x - circles[n].x) <=
                                (radius + 1) * 2 &&
                            Math.abs(position.y - circles[n].y) <=
                                (radius + 1) * 2
                        ) {
                            position = false;
                            break;
    ...
    <div id="test-body">
        <section id="StimulusArea">
            <div id="Canvases" style="width: 750px; margin:auto;">
                ...
                >
                    Use the arrow keys to indicate which circle has more dots
                    inside
                </p>
                <canvas id="Canvas0" class="left" width="310" height="310"
                ></canvas>
                <canvas id="Canvas1" class="right" width="310" height="310"
                ></canvas>
            </div>
        </section>
