export function Staircase(stairs) {
  this.stairs = {}
  for (const i in stairs) {
...
Staircase.prototype.choose = function (goodAns) {
  const stair = this.getActive()
  const ans = (goodAns)
    ? 'harder'
    : 'easier'
  if (!goodAns) { stair.startingExplorationScale = 1 } // First time we get easier we stop the inital exploration scaling.
  const wait = (stair.wait)
    ? 'wait'
    : 'noWait'
  if (stair.verbosity > 0) { console.log("Staircase '" + stair.name + "' (" + stair.operation + '): making test ' + ans + ', ' + wait + ' mode.') }
  const out = this.tasks[ans][stair.operation][wait][stair.direction](stair)
  return out
}
Staircase.prototype.checkLimits = function (currentStair) {
  const stair = currentStair
  // check limits
...
Staircase.prototype.init = function () {
  ...
  const rand = randInt(0, choices.length - 1)
...
Staircase.prototype.changeActive = function () {
  ...
  const rand = randInt(0, possibleStairs.length - 1)
...
Staircase.prototype.activate = function (stair) {
  ...
  if (this.stairs[stair].verbosity > 0) { console.log("Staircase '" + this.stairs[stair].name + "' now active") }
}
Staircase.prototype.deactivate = function (stair) {
  this.checkErr.ARG('deactivate', arguments, 1)
  this.checkErr.UNDEFINED(this.stairs, stair)
  this.stairs[stair].active = false
  if (this.stairs[stair].verbosity > 0) { console.log("Staircase '" + this.stairs[stair].name + "' deactivated") }
}
...
Staircase.prototype.getReversals = function (stair) {
  ...
  const reversals = [] // start with the first value
  let direction = this.stairs[stair].direction * -1 // start the search getting harder
  for (let i = 1; i < this.stairs[stair].val.length; i++) {
...
Staircase.prototype.reversalLimitReached = function (stair) {
  this.checkErr.ARG('getFinalVal', arguments, 1)
  this.checkErr.UNDEFINED(this.stairs, stair)
  const reversals = this.getReversals(stair)
...
Staircase.prototype.getFinalVal = function (stair) {
  ...
  this.checkErr.UNDEFINED(this.stairs, stair)
  const reversals = this.getReversals(stair)
  if (!reversals.length) { throw new Error("Staircase '" + this.stairs[stair].name + "': Not enough reversals to calculate final value.") }
  let sum = 0
  for (let i = 0; i < reversals.length; i++) { sum = sum + reversals[i] }
  return sum / reversals.length // Convergence value is the mean of the reversal points
}

const Staircase_CheckErr = function () { }
